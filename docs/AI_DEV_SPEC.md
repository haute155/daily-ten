...
- 저장 상태는 항상 표시하되 방식은 화면에 맞게: `/today`는 **자동 저장**(디바운스 600ms + 언마운트 flush + pagehide keepalive) + "자동 저장됨" 인디케이터, 나머지(설정/과거 수정)는 명시적 저장 버튼 + 토스트.
...
- [ ] AI 추천 실연동 (현재 placeholder) + **LLM 카테고리 자동 태깅** 검토 (키워드 휴리스틱은 과하다고 판단해 제거함 — 도입 시 LLM 기반으로)
- [ ] 정량형 항목 스키마 확장
